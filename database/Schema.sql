...
-- REGIONS
-- IMPORTANT:
-- "region" = code régional utilisé par l'application.
-- Exemples : RSK, CS, TTA...
-- ============================================================
...
CREATE TABLE utilisateur (
    ...
    -- Compte temporaire utilisé pour créer le premier Super Admin.
...
CREATE TABLE gestionnaire (
    ...
    KEY idx_gestionnaire_etab (id_etablissement),
...
CREATE TABLE formateur (
    ...
    KEY idx_formateur_etab (id_etablissement),
...
CREATE TABLE classe (
    ...
    KEY idx_classe_etab (id_etablissement),
    KEY idx_classe_annee (id_annee_formation),
...
CREATE TABLE stagiaire (
    ...
    UNIQUE KEY uq_numero_stagiaire (numero_stagiaire),

    KEY idx_stagiaire_etab (id_etablissement),
    KEY idx_stagiaire_classe (id_classe),
...
CREATE TABLE cours (
    ...
    KEY idx_cours_etab (id_etablissement),
    KEY idx_cours_annee (id_annee_formation),
...
CREATE TABLE affectation (
    ...
    KEY idx_affectation_classe (id_classe),
    KEY idx_affectation_cours (id_cours),
    KEY idx_affectation_formateur (id_formateur),
...
CREATE TABLE creneau (
    ...
    KEY idx_creneau_affectation (id_affectation),
    KEY idx_creneau_jour (jour_semaine, heure_debut),
...
CREATE TABLE appel (
    ...
    KEY idx_appel_creneau (id_creneau),
    KEY idx_appel_formateur (id_formateur),
...
CREATE TABLE presence (
    ...
    KEY idx_presence_stagiaire (id_stagiaire),
...
CREATE TABLE justification (
    ...
    UNIQUE KEY uq_justification_presence (id_presence),
...
CREATE TABLE document (
    ...
    KEY idx_document_gestionnaire (id_gestionnaire),
    KEY idx_document_directeur (id_directeur),
...
CREATE TABLE demande_document (
    ...
    -- Conservé pour compatibilité avec le backend actuel.
    -- Le workflow métier principal reste GS -> Directeur.
    id_administrateur CHAR(36) NULL,

    PRIMARY KEY (id_demande),

    KEY idx_demande_stagiaire (id_stagiaire),
    KEY idx_demande_gestionnaire (id_gestionnaire),
    KEY idx_demande_etab (id_etablissement),
    KEY idx_demande_document (id_document),
    KEY idx_demande_administrateur (id_administrateur),
...
CREATE TABLE annonce (
    ...
    KEY idx_annonce_cours (id_cours),
    KEY idx_annonce_classe (id_classe),
    KEY idx_annonce_auteur (id_auteur),
...
CREATE TABLE note (
    ...
    KEY idx_note_stagiaire (id_stagiaire),
    KEY idx_note_affectation (id_affectation),
    ...
    CONSTRAINT chk_note1
        CHECK (note1 IS NULL OR (note1 >= 0 AND note1 <= 20)),

    CONSTRAINT chk_note2
        CHECK (note2 IS NULL OR (note2 >= 0 AND note2 <= 20)),

    CONSTRAINT chk_note3
        CHECK (note3 IS NULL OR (note3 >= 0 AND note3 <= 20))
)
ENGINE = InnoDB
DEFAULT CHARSET = utf8mb4;
...
CREATE TABLE device_token (
    ...
    KEY idx_device_user (id_utilisateur),
...
CREATE TABLE role_permission (
    ...
    PRIMARY KEY (role, id_permission),
